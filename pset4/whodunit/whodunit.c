...
int main(int argc, char* argv[])
{
    ...
    if (inptr == NULL) // if file cannot open for reading
    {
    ...
    if (outptr == NULL) // if file cannot open for writing
    {
        fclose(inptr);
    ...
    for (int i = 0, biHeight = abs(bi.biHeight); i < biHeight; i++)
    {
        ...
        for (int j = 0; j < bi.biWidth; j++)
        {
            ...
            if(triple.rgbtRed == 0xff) // write RGB triple to outfile
            {
		        triple.rgbtRed = 0x00;
    ...
    return 0;  // if made it to this point, return 0 for success
}
