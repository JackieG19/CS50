 ...
 *                                                                        *
 * DESCRIPTION: This program demostrates drawing bitmaps,                 *
 * including transparent bitmaps.                                         *
 **************************************************************************/
...
/* load_bmp - Loads a bitmap file into memory. */
...
void load_bmp(char *file,BITMAP *b)
{
  ...
  if (num_colors==0) num_colors=256; // assume we are working with an 8-bit file
  ...
  fskip(fp,num_colors*4);  // Ignore the palette information for now
...
/* draw_transparent_bitmap - Draws a transparent bitmap. */
...
/* wait - Wait for a specified number of clock ticks (18hz). */
...
void wait(int ticks)
{
  ...
  {
    *my_clock=*my_clock;  // this line is for some compilers that would otherwise ignore this loop
  }
}

/* Main - Draws opaque and transparent bitmaps */
